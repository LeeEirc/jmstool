...
func (c *Client) handleOption(option []byte) {
	var p packet
	log.Println("server ", CodeTOASCII[option[1]], CodeTOASCII[option[2]])
	p.optionCode = option[1]
	cmd := option[2]
	p.commandCode = cmd
	switch option[1] {
	case SB:
		log.Println("sb request: ", option)
		switch cmd {
	...
	log.Println("client ", CodeTOASCII[p.optionCode], CodeTOASCII[p.commandCode])
	if err := c.replyOptionPacket(p); err != nil {
		log.Println("Handle Option err: ", err)
	}
...
func (c *Client) readOptionPacket() ([]byte, error) {
	if _, err := io.ReadFull(c.sock, c.oneByte[:]); err != nil {
		log.Println("readOptionPacket err: ", err)
		return nil, err
	}
	p := make([]byte, 0, 3)
...
func NewClientConn(c net.Conn, config *ClientConfig) (*Client, error) {
	...
	if err := conn.clientHandshake(); err != nil {
		_ = c.Close()
		return nil, fmt.Errorf("telnet: handshake failed: %v", err)
	}
	return conn, nil
}
